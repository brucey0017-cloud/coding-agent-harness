...
import { assert, assertGraphIntegrity, commandExists, expectJson, expectPass, expectTtyJson, packageVersion, run, tmpRoot, todayLocal, } from "./helpers/harness-test-utils.mjs";
...
assert(requiresCanonicalVisualMap({ migrationClassification: "historical-no-map-needed" }) === false, "historical-no-map-needed should not generate a canonical visual map action");
...
assert(dryRun.changes.some((change) => change.destination === "coding-agent-harness/governance/standards/external-source-intake-standard.md"), "init scaffold should copy the external source intake standard into v2 governance standards");
assert(dryRun.changes.some((change) => change.destination === "coding-agent-harness/planning/tasks" && change.action === "would-create-directory"), "init zh-CN dry-run should create the task root without vendoring localized package templates");
...
if (commandExists("expect")) {
    ...
    assert(interactiveZh.changes.some((change) => change.destination === "coding-agent-harness/planning/tasks" && change.action === "would-create-directory"), "interactive zh-CN init should create task root without vendoring localized package templates");
...
assert(fs.readFileSync(path.join(zhInitTarget, "coding-agent-harness/context/development/external-source-packs/README.md"), "utf8").includes("外部资料包索引"), "zh-CN init should create localized external source pack registry");
...
assert(enRun.changes.some((change) => change.destination === "coding-agent-harness/planning/tasks" && change.action === "would-create-directory"), "init en-US dry-run should create the task root without vendoring package templates");
...
assert(fs.readFileSync(path.join(enInitTarget, "coding-agent-harness/governance/standards/external-source-intake-standard.md"), "utf8").includes("External Source Intake Standard"), "en-US init should create English external source intake standard");
assert(fs.readFileSync(path.join(enInitTarget, "coding-agent-harness/context/development/external-source-packs/README.md"), "utf8").includes("External Source Packs"), "en-US init should create English external source pack registry");
...
fs.writeFileSync(path.join(invalidReviewTarget, ".harness-capabilities.json"), JSON.stringify({ version: 1, locale: "en-US", capabilities: [{ name: "core", state: "configured" }, { name: "adversarial-review", state: "configured" }] }, null, 2));
fs.writeFileSync(path.join(invalidReviewTarget, "docs/09-PLANNING/TASKS/bad/task_plan.md"), "# Bad\n");
fs.writeFileSync(path.join(invalidReviewTarget, "docs/09-PLANNING/TASKS/bad/review.md"), "# Review\n\n## Findings\n\n| ID | Severity | Finding | Evidence Checked | Required Action | Open | Disposition | Blocks Release | Follow-up |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- |\n| R-001 | P1 | Missing sections | none | fix | no | mitigated | no | next |\n");
...
fs.writeFileSync(path.join(invalidTaskStateTarget, ".harness-capabilities.json"), JSON.stringify({ version: 1, locale: "en-US", capabilities: [{ name: "core", state: "configured" }] }, null, 2));
