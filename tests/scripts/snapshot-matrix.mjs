...
export function captureSnapshotMatrix({ repoRoot = defaultRepoRoot, outDir, label = "snapshot", commands = snapshotCommands } = {}) {
  ...
  if (outDir) writeSnapshotMatrix(matrix, { repoRoot, outDir });
  return matrix;
}
...
function countTasks(value) {
  ...
  if (taskArrays.length === 0) return undefined;
...
function countNamedMetric(value, names) {
  ...
  walkValue(value, (entry, key) => {
    ...
    if (Array.isArray(entry)) counts.push(entry.length);
    else if (typeof entry === "number") counts.push(entry);
    else if (entry && typeof entry === "object") counts.push(Object.keys(entry).length);
  });
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    else if (arg === "--before-dir") options.beforeDir = rest[++index];
    else if (arg === "--after-dir") options.afterDir = rest[++index];
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (command === "capture") {
      if (!options.outDir) throw new Error("capture requires --out-dir");
      ...
      if (!options.beforeDir || !options.afterDir) throw new Error("diff requires --before-dir and --after-dir");
      ...
      const diff = compareSnapshotMatrices(before, after);
      if (options.out) fs.writeFileSync(options.out, diff.markdown);
