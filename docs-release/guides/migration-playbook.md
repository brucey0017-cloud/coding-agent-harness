...
全局表和模块索引迁移不是人工重填。`Feature-SSoT.md`、`Private-Feature-SSoT.md`、`Harness-Ledger.md`、模块 `module_plan.md` 的 Steps 表和模块 `visual_map.md` 的拓扑表在新版本里是 Agent 读取用的生成索引；人看当前状态应优先看 Dashboard，Agent 需要快速定位时用 `task-list` 查询。旧项目切换前，先把旧表归档，再用生成器从任务文件重建：
...
归档目录会保留旧表快照。确认 Dashboard、`task-list` 查询和新生成表都能反映当前任务后，再由项目 owner 决定是否删除旧归档；迁移 agent 不应直接删除历史表证据。
