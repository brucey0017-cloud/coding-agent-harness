...
import { normalizeTarget, readBundledTemplate, todayDate, toPosix, } from "./core-shared.mjs";
...
function residual(task) {
    if (Array.isArray(task.stateConflicts) && task.stateConflicts.length)
        return `state-conflicts:${task.stateConflicts.length}`;
    if (Array.isArray(task.materialIssues) && task.materialIssues.length)
        return `material-issues:${task.materialIssues.length}`;
    return "none";
}
...
function uniqueArchiveDir(target) {
    ...
    for (let index = 0; index < 100; index += 1) {
        ...
        if (!fs.existsSync(path.join(target.projectRoot, candidate)))
            return candidate;
    }
