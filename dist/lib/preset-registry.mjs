...
function assertPresetDirectory(directory) {
    if (!fs.existsSync(directory))
        throw new Error(`Preset package directory missing: ${toPosix(directory)}`);
    const stat = fs.lstatSync(directory);
    if (stat.isSymbolicLink())
        throw new Error(`Preset package directory must not be a symlink: ${toPosix(directory)}`);
    if (!stat.isDirectory())
        throw new Error(`Preset package path must be a directory: ${toPosix(directory)}`);
}
function assertPresetManifestFile(directory, manifestPath) {
    if (!fs.existsSync(manifestPath))
        throw new Error(`Preset manifest missing: ${displayManifestPath(manifestPath)}`);
    const stat = fs.lstatSync(manifestPath);
    if (stat.isSymbolicLink())
        throw new Error(`Preset manifest must not be a symlink: ${displayManifestPath(manifestPath)}`);
    if (!stat.isFile())
        throw new Error(`Preset manifest must be a file: ${displayManifestPath(manifestPath)}`);
...
export function installPresetPackage(source, { force = false, scope = "user", targetInput = ".", home = "" } = {}) {
    ...
    try {
        ...
        if (fs.existsSync(destination)) {
            if (!force)
                throw new Error(`Preset already installed: ${id}. Re-run with --force to overwrite.`);
        }
...
export function listBundledPresetIds() {
    if (!fs.existsSync(builtinPresetRoot))
        return [];
...
export function seedBundledPresets({ force = false, scope = "user", targetInput = ".", home = "", dryRun = false } = {}) {
    const presets = listBundledPresetIds().map((id) => {
        ...
        if (!dryRun && (!existsAlready || force))
            copyPresetPackage(sourcePath, destination, stagedPreset.id);
        return {
            id: stagedPreset.id,
...
export function validatePresetPackage(preset) {
    ...
    if (!Number.isInteger(preset.version))
        failures.push("missing numeric version");
    if (!preset.compatibleBudgets.length)
        failures.push("missing compatibleBudgets");
    if (!preset.audit.manifestRequired)
        failures.push("audit.manifestRequired must be true");
    if (!preset.writeScopes.length)
        failures.push("missing writeScopes");
    for (const [name, input] of Object.entries(preset.inputs)) {
        if (!["text", "flag", "json-file"].includes(input.type))
            failures.push(`${name} has unsupported input type: ${input.type || "(missing)"}`);
        if (!input.flag && input.type !== "flag")
            warnings.push(`${name} input has no CLI flag`);
    }
    if (preset.evidence?.bundleDir && unsafeRelativePresetPath(preset.evidence.bundleDir))
        failures.push(`evidence.bundleDir escapes task directory: ${preset.evidence.bundleDir}`);
    ...
    for (const [name, evidence] of Object.entries(preset.evidence?.files || {})) {
        ...
        if (evidence.path && unsafeRelativePresetPath(evidence.path))
            failures.push(`evidence file ${name} path escapes evidence bundle: ${evidence.path}`);
        if (evidence.type && !allowedEvidenceTypes.has(String(evidence.type)))
            failures.push(`evidence file ${name} has unsupported type: ${evidence.type}`);
    }
    ...
    for (const requiredRead of preset.context?.requiredReads || []) {
        if (!referenceIds.has(requiredRead))
            failures.push(`required read ${requiredRead} does not match a declared reference`);
    }
    for (const [name, entrypoint] of Object.entries(preset.entrypoints)) {
        if (!allowedEntrypoints.has(name))
            failures.push(`unsupported entrypoint: ${name}`);
        if (!allowedEntrypointTypes.has(entrypoint.type))
            failures.push(`${name} has unsupported type: ${entrypoint.type || "(missing)"}`);
    ...
    for (const [templateKey, templatePath] of Object.entries(preset.newTaskTemplates)) {
        ...
        if (!isInside(preset.directory, absolute))
            failures.push(`template escapes preset package: ${templatePath}`);
        else
            validatePresetPackageFile(preset, templatePath, "template", failures);
    }
...
function validateResourceCollection(preset, label, groupName, requiredPrefix, resourcePaths, failures) {
    ...
    for (const [name, resource] of Object.entries(preset.resources?.[groupName] || {})) {
        ...
        if (!resource.path)
            failures.push(`${label} resource ${name} missing path`);
        else if (hasMarkdownTableDelimiter(resource.path))
            failures.push(`${label} resource ${name} path cannot contain Markdown table delimiters: ${resource.path}`);
        else if (unsafeRelativePresetPath(resource.path))
            failures.push(`resource ${name} path escapes task directory: ${resource.path}`);
        ...
        if (!resource.source && !resource.template)
            failures.push(`${label} resource ${name} missing source or template`);
        if (resource.source && resource.template)
        ...
        if (!id)
            failures.push(`${label} resource ${name} missing index.id`);
        if (id && hasMarkdownTableDelimiter(id))
            failures.push(`${label} resource ${name} index.id cannot contain Markdown table delimiters: ${id}`);
        if (id && seen.has(id))
            failures.push(`duplicate ${label} resource id: ${id}`);
        if (id)
            seen.add(id);
    }
}
function validateAuditEvidenceFiles(preset, failures) {
    ...
    for (const name of preset.audit?.evidenceFiles || []) {
        ...
        if (!raw)
            failures.push("audit evidence file name is empty");
        else if (hasMarkdownTableDelimiter(raw))
            failures.push(`audit evidence file cannot contain Markdown table delimiters: ${raw}`);
...
function validatePresetPackageFile(preset, relativePath, label, failures) {
    ...
    if (!isInside(realRoot, realPath))
        failures.push(`${label} real path escapes preset package: ${relativePath}`);
}
export function parseSimpleYaml(source) {
    ...
    for (const rawLine of String(source).split(/\r?\n/)) {
        ...
        if (!match)
            throw new Error(`Unsupported preset YAML line: ${rawLine}`);
...
function presetSearchRoots({ targetInput = "", home = "" } = {}) {
    const roots = [];
    if (targetInput)
        roots.push({ source: "project", root: projectPresetRoot(targetInput) });
...
function presetRootFromExtractedArchive(tempRoot) {
    if (fs.existsSync(path.join(tempRoot, "preset.yaml")))
        return tempRoot;
...
function extractPresetZip(sourcePath, destinationRoot) {
    ...
    if (archiveStat.size > maxPresetArchiveBytes)
        throw new Error("Preset archive file is too large.");
    ...
    if (entryCount > maxPresetArchiveEntries)
        throw new Error(`Preset archive has too many entries: ${entryCount}`);
    if (centralOffset + centralSize > archive.length)
        throw new Error("Invalid preset archive central directory.");
    const written = new Set();
    let cursor = centralOffset;
    let totalUncompressed = 0;
    for (let index = 0; index < entryCount; index += 1) {
        if (archive.readUInt32LE(cursor) !== 0x02014b50)
            throw new Error("Invalid preset archive central directory entry.");
        ...
        if (shouldSkipZipEntry(rawName))
            continue;
        if (flags & 0x0001)
            throw new Error(`Encrypted preset archive entries are not supported: ${rawName}`);
        if (method !== 0 && method !== 8)
            throw new Error(`Unsupported preset archive compression method ${method}: ${rawName}`);
        const mode = (externalAttributes >>> 16) & 0o170000;
        if (mode === 0o120000)
            throw new Error(`Preset archive must not contain symlinks: ${rawName}`);
        ...
        if (written.has(entryName))
            throw new Error(`Preset archive contains duplicate entry: ${entryName}`);
        if (uncompressedSize > maxPresetArchiveUncompressedBytes - totalUncompressed)
            throw new Error("Preset archive is too large.");
        ...
        if (!isInside(path.resolve(destinationRoot), destination))
...
function readZipEntryData(archive, { localOffset, compressedSize, uncompressedSize, method, name }) {
    ...
    if (dataEnd > archive.length)
        throw new Error(`Invalid preset archive entry size: ${name}`);
    ...
    if (data.length !== uncompressedSize)
        throw new Error(`Preset archive entry size mismatch: ${name}`);
    return data;
}
...
function safeZipEntryName(rawName) {
    if (String(rawName).includes("\0"))
        throw new Error("Preset archive entry contains NUL byte.");
