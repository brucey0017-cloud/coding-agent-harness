...
import { acceptNoLessonCandidate, assert, expectJson, expectPass, run, tmpRoot, todayLocal, } from "../helpers/harness-test-utils.mjs";
...
fs.writeFileSync(reviewPath, replaceHumanConfirmationSection(afterSubmitReview, [
    "## Human Review Confirmation",
    "",
    "| Field | Value |",
    "| --- | --- |",
...
fs.writeFileSync(reviewPath, afterSubmitReview
    .replace("| ID | Severity | Finding | Evidence Checked | Required Action | Open | Disposition | Blocks Release | Follow-up |", "| ID | 严重级别 | 发现 | 已检查证据 | Required Action | 是否开放 | 处置 | 是否阻塞发布 | Follow-up |")
...
fs.writeFileSync(path.join(lessonDir, "lesson_candidates.md"), [
...
assert(missingDetailArtifactTask.lessonCandidateIssues.includes("missing-detail-artifact:LC-QUEUE-LESSON:lessons/LC-QUEUE-LESSON.md"), "needs-promotion lesson rows should require the task-local detail artifact file to exist");
fs.mkdirSync(path.join(lessonDir, "lessons"), { recursive: true });
fs.writeFileSync(path.join(lessonDir, "lessons/LC-QUEUE-LESSON.md"), [
...
fs.appendFileSync(path.join(supersededDir, "task_plan.md"), [
    "",
    "## Task Tombstone",
    "",
    "| Field | Value |",
