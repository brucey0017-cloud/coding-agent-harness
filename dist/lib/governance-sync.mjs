...
export function releaseGovernanceSync(context) {
    if (!context?.active || !context.lockPath)
        return;
    try {
...
export function commitGovernanceSync(context, allowedRelativePaths, { message = "chore(harness): sync governance state" } = {}) {
    const allowed = [...new Set((allowedRelativePaths || []).filter(Boolean).map(toPosix))].sort();
    if (context?.dryRun || !context?.git?.inGit)
        return { committed: false, reason: context?.git?.inGit ? "dry-run" : "not-git", allowedPaths: allowed };
    if (allowed.length === 0)
        return { committed: false, reason: "no-allowed-paths", allowedPaths: allowed };
    ...
    if (staged.length === 0)
        return { committed: false, reason: "no-changes", allowedPaths: allowed };
...
export function inspectGit(root) {
    ...
    if (gitRootResult.status !== 0)
        return { inGit: false, gitRoot: "", entries: [] };
...
function assertNoUnexpectedOutsideChanges(root, allowedPaths, initialDirtyEntries) {
    const allowed = new Set(allowedPaths);
    const initialByPath = new Map((initialDirtyEntries || [])
